...
--
-- Host: 127.0.0.1
-- Waktu pembuatan: 06 Mar 2023 pada 18.25
...
INSERT INTO `tb_barang` (`b_id`, `b_nama`, `k_id`, `u_id`) VALUES
(1, 'Fanta', 4, 1),
(4, 'coca cola', 4, 1),
(5, 'Sprit', 4, 1),
(15, 'Ciki Aries', 7, 1),
...
CREATE TABLE `tb_cicilan` (
  ...
  `c_cicilan_masuk` bigint(255) NOT NULL,
  `c_cicilan_outstanding` bigint(255) NOT NULL,
  `c_total_biaya` bigint(255) NOT NULL,
  `c_biaya_masuk` bigint(255) NOT NULL,
  `c_biaya_outstanding` bigint(255) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
...
INSERT INTO `tb_kategori` (`k_id`, `k_nama`, `u_id`) VALUES
(1, 'Makanan', 1),
(4, 'Minuman', 1),
(7, 'Snack', 1);
...
INSERT INTO `tb_kategori_paket` (`kp_id`, `kp_nama`) VALUES
(1, 'MINI'),
(2, 'MIDI'),
(3, 'JUMBO'),
(4, 'SUPER'),
...
CREATE TRIGGER `delete_c_cicilan_masuk` AFTER DELETE ON `tb_log_cicilan` FOR EACH ROW BEGIN
  UPDATE tb_cicilan
  SET c_cicilan_masuk = c_cicilan_masuk - OLD.l_jumlah_bayar
  WHERE c_id = OLD.c_id;
END
$$
DELIMITER ;
...
CREATE TRIGGER `delete_c_cicilan_outstanding` AFTER DELETE ON `tb_log_cicilan` FOR EACH ROW BEGIN
  UPDATE tb_cicilan
  SET c_cicilan_outstanding = c_cicilan_outstanding + OLD.l_jumlah_bayar
  WHERE c_id = OLD.c_id;
END
$$
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `delete_c_total_cicilan` AFTER DELETE ON `tb_log_cicilan` FOR EACH ROW BEGIN
  UPDATE tb_cicilan
  SET c_total_cicilan = c_total_cicilan + OLD.l_jumlah_bayar
  WHERE c_id = OLD.c_id;
END
$$
DELIMITER ;
...
CREATE TRIGGER `report_c_cicilan_masuk` AFTER INSERT ON `tb_log_cicilan` FOR EACH ROW BEGIN
  UPDATE `tb_cicilan` SET `c_cicilan_masuk` = `c_cicilan_masuk` + NEW.`l_jumlah_bayar`
  WHERE `c_id` = NEW.`c_id`;
END
$$
DELIMITER ;
...
CREATE TRIGGER `report_c_cicilan_outstanding	` AFTER INSERT ON `tb_log_cicilan` FOR EACH ROW BEGIN
  UPDATE `tb_cicilan` SET `c_cicilan_outstanding` = `c_cicilan_outstanding` - NEW.`l_jumlah_bayar`
  WHERE `c_id` = NEW.`c_id`;
END
$$
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `report_c_total_cicilan` AFTER INSERT ON `tb_log_cicilan` FOR EACH ROW BEGIN
  UPDATE `tb_cicilan` SET `c_total_cicilan` = `c_total_cicilan` - NEW.`l_jumlah_bayar`
  WHERE `c_id` = NEW.`c_id`;
END
$$
DELIMITER ;
...
INSERT INTO `tb_paket` (`p_id`, `kp_id`, `pe_id`, `p_hargaJual`, `p_hargaBarang`, `pa_id`, `p_cashback`, `p_insentive`, `p_laba`, `p_persentaseLaba`, `u_id`, `created_at`) VALUES
(24, 20, 4, 1625000, 350000, 18, 1000000, 57500, 167500, '10.307692307692', 1, '2023-03-06 22:56:13');
...
CREATE TABLE `tb_pengambilan_paket` (
  ...
  `p_sb_id` int(11) NOT NULL,
  `p_pa_id` int(11) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
...
INSERT INTO `tb_pengambilan_paket` (`pp_id`, `pp_p_id`, `p_sb_id`, `p_pa_id`) VALUES
(37, 24, 18, 18);
...
INSERT INTO `tb_supplier` (`s_id`, `s_nama`, `s_alamat`, `s_contact`, `u_id`) VALUES
(1, 'PT. Sprit', 'cimindi', '0822173987771', 1),
(4, 'PT. Coca-Cola', 'cimindi', '0822173987771', 1),
(5, 'PT. Snack ', 'Cimindi', '0895353025503', 1),
(6, 'PT. Oreo', 'Jl. Industri', '087730381592', 1),
(7, 'PT. Daging Sapi', 'Jl. Cibaligo', '0895 3530 25503', 1),
(8, 'PT. Indofood', 'Padalarang', '0877 3038 1592', 1);

-- --------------------------------------------------------

--
-- Struktur dari tabel `tb_sup_barang`
--

CREATE TABLE `tb_sup_barang` (
  `sb_id` int(11) NOT NULL,
  `s_id` int(11) NOT NULL,
  `b_id` int(11) NOT NULL,
  `sb_hargaasli` bigint(255) NOT NULL,
  `sb_hargajual` bigint(255) NOT NULL,
  `sb_qty` int(11) NOT NULL,
  `sb_berat/ukuran` int(11) NOT NULL,
  `sb_ktrg_berat/ukuran` varchar(11) NOT NULL,
  `sb_foto` varchar(255) DEFAULT NULL,
  `u_id` int(11) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data untuk tabel `tb_sup_barang`
--

INSERT INTO `tb_sup_barang` (`sb_id`, `s_id`, `b_id`, `sb_hargaasli`, `sb_hargajual`, `sb_qty`, `sb_berat/ukuran`, `sb_ktrg_berat/ukuran`, `sb_foto`, `u_id`) VALUES
(18, 4, 5, 5000, 7000, 23, 1, 'liter', '1675827697_a25ccfe4e03e9d5e9060.jpg', 1),
(21, 5, 15, 5000, 700000, 23, 9, 'gram', '1675834530_554a826cb61c5ef9ad35.jpg', 1),
(22, 7, 154, 75000, 100000, 23, 11, 'kg', '1675839213_b642dc1eadff77e70691.jpg', 1),
(23, 6, 16, 70000, 140000, 23, 11, 'gram', '1678082109_409c7faa5feab34421e5.jpg', 1),
(24, 8, 155, 6500, 7000, 23, 400, 'ml', '1678121974_0190a9f5dbd1c268cbe3.jpg', 1);
...
CREATE TABLE `tb_user` (
  ...
  `u_role` enum('admin','owner','coordinator','anggota') NOT NULL,
  ...
  `u_provinsi` varchar(100) NOT NULL,
  `u_kota` varchar(100) NOT NULL,
  `u_kelurahan` varchar(100) NOT NULL,
  `u_kecamatan` varchar(100) NOT NULL,
...
INSERT INTO `tb_user` (`u_id`, `u_username`, `u_password`, `u_fullname`, `u_role`, `u_referensi`, `u_email`, `u_create_at`, `u_nik`, `u_nama`, `u_tempat_lahir`, `u_tanggal_lahir`, `u_jenis_kelamin`, `u_provinsi`, `u_kota`, `u_kelurahan`, `u_kecamatan`, `u_kodepos`) VALUES
(1, 'admin', '$2y$10$7x/8I8KIGD3cggE00uGRQuSa0q6AlG3PFZL2.aEXEs46f9HWFzn1S', 'Ahmad Arif', 'admin', 1, 'qvftp1223@gmail.com', '2023-02-20 01:55:40', 2147483647, 'Ahmad Arif', 'Cimahi', '2023-01-23', 'Laki-laki', '32', '3277', 'Cibeureum', '3277010', '10730'),
(2, 'coordinator', '$2y$10$6HEJ/LZzn0C5xmiqzF/WDO8tQiwytQdaiTL6vm7CJMKGWTBvG6Y3S', 'coordinator', 'coordinator', 1, 'coordinator@gmail.com', '2023-02-20 01:54:34', 12321, 'coordinator', 'asdsad', '2023-01-23', 'Laki-laki', '32', '3277', 'Cibeureum', '3277010', '40535'),
(71, 'owner', '$2y$10$Dixj6X6XBdDShEStbef3Qed49krKjF3haZ1xnJsU5aq8T.1PHcTFy', 'Dadang', 'owner', 1, 'Aseppp@gmail.com', '2023-02-20 01:00:27', 2147483647, 'Aseppp', 'Cimahi', '2023-02-20', 'Laki-laki', '32', '3277', 'Cibeureum', '3277010', '40535');
...
ALTER TABLE `tb_pengambilan_paket`
  ...
  ADD KEY `p_id` (`pp_p_id`,`p_sb_id`),
  ADD KEY `sb_id` (`p_sb_id`),
  ADD KEY `pa_id` (`p_pa_id`);
...
--
-- Indeks untuk tabel `tb_sup_barang`
--
ALTER TABLE `tb_sup_barang`
  ADD PRIMARY KEY (`sb_id`),
  ADD KEY `s_id` (`s_id`,`b_id`),
  ADD KEY `b_id` (`b_id`),
  ADD KEY `u_id` (`u_id`);
...
--
ALTER TABLE `tb_barang`
  MODIFY `b_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=156;
...
--
ALTER TABLE `tb_kategori_paket`
  MODIFY `kp_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=26;
...
--
ALTER TABLE `tb_packaging`
  MODIFY `pa_id` int(30) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=25;
...
--
-- AUTO_INCREMENT untuk tabel `tb_sup_barang`
--
ALTER TABLE `tb_sup_barang`
  MODIFY `sb_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=26;
...
--
ALTER TABLE `tb_barang`
  ADD CONSTRAINT `tb_barang_ibfk_1` FOREIGN KEY (`k_id`) REFERENCES `tb_kategori` (`k_id`),
  ADD CONSTRAINT `tb_barang_ibfk_2` FOREIGN KEY (`u_id`) REFERENCES `tb_user` (`u_id`);
...
--
ALTER TABLE `tb_kategori`
  ADD CONSTRAINT `tb_kategori_ibfk_1` FOREIGN KEY (`u_id`) REFERENCES `tb_user` (`u_id`);
...
ALTER TABLE `tb_paket`
  ...
  ADD CONSTRAINT `tb_paket_ibfk_6` FOREIGN KEY (`kp_id`) REFERENCES `tb_kategori_paket` (`kp_id`);
...
ALTER TABLE `tb_pengambilan_paket`
  ...
  ADD CONSTRAINT `tb_pengambilan_paket_ibfk_2` FOREIGN KEY (`p_sb_id`) REFERENCES `tb_sup_barang` (`sb_id`),
  ADD CONSTRAINT `tb_pengambilan_paket_ibfk_3` FOREIGN KEY (`p_pa_id`) REFERENCES `tb_packaging` (`pa_id`);

--
-- Ketidakleluasaan untuk tabel `tb_supplier`
--
ALTER TABLE `tb_supplier`
  ADD CONSTRAINT `tb_supplier_ibfk_1` FOREIGN KEY (`u_id`) REFERENCES `tb_user` (`u_id`);

--
-- Ketidakleluasaan untuk tabel `tb_sup_barang`
--
ALTER TABLE `tb_sup_barang`
  ADD CONSTRAINT `tb_sup_barang_ibfk_1` FOREIGN KEY (`b_id`) REFERENCES `tb_barang` (`b_id`),
  ADD CONSTRAINT `tb_sup_barang_ibfk_3` FOREIGN KEY (`u_id`) REFERENCES `tb_user` (`u_id`),
  ADD CONSTRAINT `tb_sup_barang_ibfk_4` FOREIGN KEY (`s_id`) REFERENCES `tb_supplier` (`s_id`);
